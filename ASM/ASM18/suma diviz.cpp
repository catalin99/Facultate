...
void main()
{
	...
	_asm
	{
		MOV EBX, s //initializam EBX (suma) cu 0
		MOV ECX, divizor // punem in ECX "i"-ul care parcurge divizorii (de la 2 la n-1)
		_start: //for-ul
		MOV EAX, numar // punem numarul nostru in EAX
		CMP ECX, EAX ///
			JGE _end_of_start // Daca ECX (i-ul pt parcurgere) este >= EAX ...JUMP la (*)
				//altfel programul ruleaza in continuare
				MOV EDX, 0 //Initializarea restului cu 0
				DIV ECX //Imparte EAX la ECX (numarul la i-ul pt parcurgerea divizorilor)
				CMP EDX, 0 ///
					JNE _not_egale //Daca EDX e diferit de 0 (daca restul nu este null)...JUMP la (**)
						//altfel actualizeaza suma
						ADD EBX, ECX //EBX+ECX
					_not_egale: // (**)
				INC ECX //Creste i-ul
				JMP _start //reia for-ul de la inceput
			_end_of_start: //(*)
		MOV s, EBX // finaaal
