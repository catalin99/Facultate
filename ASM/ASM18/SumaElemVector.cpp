...
void main()
{
	...
	_asm
	{
		MOV ECX, nr //nr elemente
		XOR EAX, EAX //indice de parcurgere //initializare cu 0
		LEA EDX, v //memoram vectorul in edx
		MOV ESI, 0 //suma=0
		_parc: //for
		CMP ECX, EAX 
			JE _stop_parc
			MOV EBX, [EDX+4*EAX] //muta in ebx elementul respectiv din vector (*4, pt ca int are 4bytes)
			ADD ESI, EBX
			INC EAX
			JMP _parc
			_stop_parc:
