...
int sumaf(int)
{
	_asm
	{
		PUSH 0 //Adaugam o variabila in stiva ESP in care se retine suma + DEC ESP, 4
		MOV EAX, 1 //a
		MOV EBX, 2 //b
		MOV ECX, [EBP+8] //n
		_while:
			CMP ECX, 0
				JZ _end_while
			_else:
				XOR EDX, EDX // EDX=0, pentru inmultire
				MUL EBX
				ADD [ESP], EAX
				ADD EBX, 2
				DEC ECX
				JMP _while
		_end_while:
		POP EAX //echivalent cu cele doua instructiuni de mai jos
		//MOV EAX, [ESP] //salveaza valoarea din stiva in EAX pentru returnare
...
void main()
{
	...
	_asm
	{
		push n //**Dec esp cu 4 si adauga n in stiva
		call sumaf //apelarea functiei
		MOV suma, EAX
		pop [ESP]//**refaace ESP/INC esp cu 4 //goleste stiva
	}
	cout << suma;
#ifdef newline
	newline;
