...
---------------------join 
select * from emp JOIN dept ON dept.deptno = emp.deptno;--vizualizare plan join fara indecsi
...
select * from emp JOIN dept ON dept.deptno = emp.deptno; -- vizualizare plan join si cost cu index
...
begin
DBMS_STATS.GATHER_INDEX_STATS('SYSTEM','dept_deptno_idx');
end;
begin
DBMS_STATS.GATHER_TABLE_STATS('SYSTEM','emp');
end;
